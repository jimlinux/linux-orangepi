...
// 主要功能：
// 把cfs_rq插入到链表rq->leaf_cfs_rq_list中
// 规则：
// 		1. 子cfs_rq在父cfs_rq之前
//		2. top cfs_rq在链表末尾
//		3. 当cfs_rq负载为0后，删除
//		4. 调用者在循环中调用此函数：从cfs_rq开始沿着group树向上把父cfs_rq都插入
// 作用：保存rq上所有有负载的cfs_rq, 用来update负载，例 __update_blocked_fair
// 调用者：propagate_entity_cfs_rq ... 
static inline bool list_add_leaf_cfs_rq(struct cfs_rq *cfs_rq)
{
	struct rq *rq = rq_of(cfs_rq);
	int cpu = cpu_of(rq);

	// tmp_alone_branch已经重置为rq->leaf_cfs_rq_list
	// 表示从下向上add leaf处理已结束
	if (cfs_rq->on_list)
	...
	if (cfs_rq->tg->parent &&
		 ...
		 */
		// 由于双向循环链表结构，放在parent->leaf_cfs_rq_list末尾即是
		// 在它前一个
		 ...
		 * list.
		 */
		// 找到父节点在链表上，重置tmp_alone_branch，处理结束
	 ...
	 */
	// tmp_alone_branch用做处理中临时指针，
	// 例：propagate_entity_cfs_rq
	// 最开始rq->tmp_alone_branch == rq->leaf_cfs_rq_list
	// 插入一个cfs_rq后，rq->tmp_alone_branch指向这个cfs_rq,
	// 继续向上操作父cfs_rq, 保证父节点放在子节点之后
	// 等从下向上处理完后，重置rq->tmp_alone_branch = rq->leaf_cfs_rq_list
...
static inline void update_tg_load_avg(struct cfs_rq *cfs_rq)
{
	...
	if (abs(delta) > cfs_rq->tg_load_avg_contrib / 64) {
		// 更新tg负载，tg->load_avg记录cfs_rq[cpu]负载之和
...
static inline void add_tg_cfs_propagate(struct cfs_rq *cfs_rq, long runnable_sum)
{
	// 标记cfs_rq有需要向上同步的avg
	cfs_rq->propagate = 1;
	// 数值是新se的avg.load_sum，放在cfs_rq->prop_runnable_sum
	cfs_rq->prop_runnable_sum += runnable_sum;
}

/* Update task and its cfs_rq load average */
// 主要功能：处理group se下的gcfs_rq->prop_runnable_sum传上来的avg变化，向上cfs_rq传播
static inline int propagate_entity_load_avg(struct sched_entity *se)
{
	...
	// 由于Group se下的gcfs_rq中新增se带来负载变化，需要更新group se和group se上的cfs_rq的负载：

	// Group se的util跟随其下gcfs_rq的util，同时把新增的util加到其上的cfs_rq
	update_tg_cfs_util(cfs_rq, se, gcfs_rq);
	// Group se的runnable跟随其下gcfs_rq的runnable，同时把新增的util加到其上的cfs_rq
	update_tg_cfs_runnable(cfs_rq, se, gcfs_rq);
	// Group se的load跟随其下gcfs_rq的load，同时把新增的util加到其上的cfs_rq
 ...
 */
// 主要作用：更新cfs rq负载
// 返回值：decayed，表示now已经进入了新负载计算周期(一个周期=1024us)
static inline int
 ...
 */
//  主要功能：把se的avg加到cfs_rq, 并且记录load_avg到prop_runnable_sum，用来向上传递
...
/* Update task and its cfs_rq load average */
// 主要功能：更新cfs_rq和se的avg，如果se是新加入到cfs_rq的，则需要向上传递变动；
static inline void update_load_avg(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	...
	if (!se->avg.last_update_time && (flags & DO_ATTACH)) {
		 ...
		 */
		// 4. 把se负载attach到cfs_rq负载
		attach_entity_load_avg(cfs_rq, se);
		// 5. cfs rq avg更新到其所属的task group
...
static void sync_entity_load_avg(struct sched_entity *se)
{
	...
	// delta = cfs_rq->last_update_time - se->last_update_time
...
// todo:
static void set_burst_state(struct cfs_rq *cfs_rq, bool state) {
	struct rq *rq = rq_of(cfs_rq);
	if (state) {
...
void reset_bursted_cfs_rq(struct cfs_rq *cfs_rq) {
	struct rq *rq = rq_of(cfs_rq);
	if (!cfs_rq->bursted && rq->nr_bursted_cfs_rq > 0) {
		struct cfs_rq *boosted_cfs_rq;
		rcu_read_lock();
		list_for_each_entry_rcu(boosted_cfs_rq, &rq->bursted_cfs_rq,
					boosted_rq_list) {
			// must check boosted flag, maybe alreday del in throttle_cfs_rq
			if(boosted_cfs_rq->bursted) {
				boosted_cfs_rq->runtime_remaining -= boosted_cfs_rq->runtime_boosted;
				boosted_cfs_rq->runtime_boosted = 0;
				if (!boosted_cfs_rq->curr)
					throttle_cfs_rq(boosted_cfs_rq);
			}
		}
		rcu_read_unlock();
	}
...
static int do_sched_cfs_period_timer(struct cfs_bandwidth *cfs_b, int overrun, unsigned long flags)
{
	...
	rcu_read_lock();
	list_for_each_entry_rcu(cfs_rq, &cfs_b->bursted_cfs_rq,
				boosted_list) {
		struct rq *rq = rq_of(cfs_rq);
		struct rq_flags rf;
		...
		if(cfs_rq->bursted) {
			set_burst_state(cfs_rq, false);
			// todo, runtime_remaining恢复后<=0
			// 这里要从cfs_b->runtime 分配时间，使runtime_remaining>0
			cfs_rq->runtime_remaining -= cfs_rq->runtime_boosted;
			cfs_rq->runtime_boosted = 0;
			list_del_rcu(&cfs_rq->boosted_rq_list);
			rq->nr_bursted_cfs_rq--;
...
static void __return_cfs_rq_runtime(struct cfs_rq *cfs_rq)
{
	...
	if(cfs_b->burst_idle && cfs_rq->bursted)
		return;
...
static u64 distribute_cfs_runtime_boost(struct rq *cur_rq)
{
	...
	list_for_each_entry_rcu(cfs_rq, &cur_rq->throttled_cfs_rq,
		...
		// add to cfs_b bursted_cfs_rq
		list_add_tail_rcu(&cfs_rq->boosted_list,
				  &cfs_b->bursted_cfs_rq);

		raw_spin_unlock_irqrestore(&cfs_b->lock, flags);

		// add to rq bursted_cfs_rq
		list_add_tail_rcu(&cfs_rq->boosted_rq_list,
				  &cur_rq->bursted_cfs_rq);

		cur_rq->nr_bursted_cfs_rq++;

		set_burst_state(cfs_rq, true);
...
void init_cfs_bandwidth(struct cfs_bandwidth *cfs_b)
{
	...
	INIT_LIST_HEAD(&cfs_b->bursted_cfs_rq);
...
static void init_cfs_rq_runtime(struct cfs_rq *cfs_rq)
{
	...
	INIT_LIST_HEAD(&cfs_rq->boosted_rq_list);
}
...
enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags)
{
	...
	reset_bursted_cfs_rq(cfs_rq_of(se));
...
wake_affine_idle(int this_cpu, int prev_cpu, int sync)
{
	 ...
	 * on one CPU.
	 */
	//若正在执行的cpu是idle的(说明p是被中断唤醒)，
	//且this cpu和prev cpu属于同一个MC domain时选this cpu
		   ...
		   int this_cpu, int prev_cpu, int sync)
{
	...
	if (sched_feat(WA_BIAS))
		this_eff_load *= 100;
	//再乘以cpu算力中可用于cfs任务的算力(除去/irq/rt/dl占用后的算力)，乘以的竟然是prev_cpu的！
...
/*
 * 主要功能：在最idle的sched group中找出最idle的cpu, 若是没找到就返回当前cpu.
 * 只有fork或exec的选核才可能走这个函数。
 *
 * 参数：
 * 	@sd: 当前cpu对应的DIE层级的sd(只可能是DIE层级);
 * 	@p: 待选核任务;
 *  @cpu: 当前运行这个函数的cpu;
 * 	@prev_cpu: p之前运行的cpu;
 * 	@sd_flag: 表示哪种选核类型
				  ...
				  int cpu, int prev_cpu, int sd_flag)
{
	...
	// p->cpus_ptr是p设置的cpu亲和性，如果sd的cpu都不在里面
	...
	while (sd) {
		...
		// 1. sd中找idlest group，如果local group也比较idle，则优先选local gorup
		...
		// 2. 在idlest sched_group找idlest的cpu
		...
		// 3. 下面逻辑主要是看，如果上面选出来的new_cpu是在DIE中选出来的，
		// 那么还要在new_cpu所在的MC sd中再选一次更优
		cpu = new_cpu;
 ...
 */
// 主要功能: 在target所在的MC sd中所有cpu中，找idle
static int select_idle_cpu(struct task_struct *p, struct sched_domain *sd, int target)
{
	...
	// 由于此函数走的是fast path，所以要度量在this cpu运行此函数的cost < avg_idle
	// 如果cost较大，就算了；
 ...
 * maximize capacity.
 */
// 主要功能：从target_cpu开始遍历，找一个能容纳下任务p的idle cpu, 
// 若所有idle cpu都不能容纳下任务，就返回算力最大的idle cpu,
static int
...
// 主要功能：
// 参数：
//  @target: 可能是prev_cpu或者wake_affine选出来的cpu(prev或者this)
//  @prev：给p选核前p所在的cpu
// 主要策略：
// 1. target为idle，选target
// 2. prev为idle且与target share cache，选prev
// 3. p->recent_used_cpu为idle且与target share cache，选recent_used_cpu
// 4. 非对称cpus，DIE sd有idle cpu，选idle
// 5. target->sd中，有idle选idle
// 6. 没有idle，选target
static int select_idle_sibling(struct task_struct *p, int prev, int target)
{
	...
	// 1. target idle，且满足p的算力要求，就选target吧
	 ...
	 */
	// 2. prev idle, 且与target share cache，且满足算力要求，就选prev吧
     ...
     *
     * 3. 若 p->recent_used_cpu 既不是prev_cpu也不是target_cpu, 
	 * 但是是和target_cpu共cluster的，且是idle的, 且任务的亲和性允许，
	...
	if (static_branch_unlikely(&sched_asym_cpucapacity)) {
		 ...
		 * capacity path.
		 */
		// 4. 如果是异构架构，如大小核，在DIE范围内有idle还是优先用idle，这时候不考虑cpu cache了；
		// 遍历DIE sd内cpu，找一个能容纳下任务p的idle cpu, 
		// 若所有idle cpu都不能容纳下任务，就返回算力最大的idle cpu
		if (sd) {
...
sd_llc:
	...
	// smt范围内找idle，arm上pass
	i = select_idle_core(p, sd, target);
	if ((unsigned)i < nr_cpumask_bits)
		return i;

	// 5. 在同一个MC sd中选择idle的cpu
 ...
 * to @dst_cpu.
 */
// 主要功能：计算cpu（参数1）上cfs_rq的util负载
// 如果cpu是p的prev cpu，则减去p的util
// 如果cpu是p放入的目标cpu，则加上p的util
 ...
 * task.
 */
// 主要功能：评估把p放到dst_cpu后，性能域pd上的功耗
static long
compute_energy(struct task_struct *p, int dst_cpu, struct perf_domain *pd)
{
	struct cpumask *pd_mask = perf_domain_span(pd);
	// cpumask_first 获取mask中第一位
	// 获取cpu的capicity
	...
	for_each_cpu_and(cpu, pd_mask, cpu_online_mask) {
		 ...
		 */
		// cpu_cap作为max，计算cfs,rt,dl,irq总util
 ...
 */
// 主要功能：找能效最高的cpu
// 策略：找算力最富余的cpu，把task尽量均匀放在一个cpu cluster中，其他cpu可以降频进入idle省电
// 具体方法：
// 1.在小核pd域中找剩余算力最大的核，计算把p放进去后，小核pd的总功耗
// 2.在大核pd域中找剩余算力最大的核，计算把p放进去后，大核pd的总功耗
// 上面2个选功耗低的核
// 3. 对于latency_sensitive，不考虑功耗，选最idle或者剩余算力最多的cpu
// 注意：只对wakeup进程走此流程，因为新进程没有运行数据，无法评估其负载贡献
static int find_energy_efficient_cpu(struct task_struct *p, int prev_cpu, int sync)
{
	...
	// root domain overutilized， 系统负荷已经很大，不用考虑省电了
	if (!pd || READ_ONCE(rd->overutilized))
		goto fail;

	cpu = smp_processor_id();
	// 满足sync唤醒 且当前cpu只有waker运行，且cpu满足wakee算力需求，就用当前cpu
	 ...
	 */
	// 包含SD_ASYM_CPUCAPACITY flag的最低sd，
	// SD_ASYM_CPUCAPACITY表示域内有非对称cpu，orangepi上是DIE domain
	...
	for (; pd; pd = pd->next) {
		...
		for_each_cpu_and(cpu, perf_domain_span(pd), sched_domain_span(sd)) {
			 ...
			 */
			// 1. max_spare_cap_cpu 此pd中记录最大剩余算力的cpu
			...
			// 3.1 这个if else用来在所有pd中找best_idle_cpu和max_spare_cap_cpu_ls
			// 这两个用来给latency_sensitive p找cpu，不考虑功耗
			if (idle_cpu(cpu)) {
				// 全范围（指所有pd覆盖的cpu中）找最佳idle cpu
				cpu_cap = capacity_orig_of(cpu);
				// p boosted：选最大算力的idle cpu
				if (boosted && cpu_cap < target_cap)
					continue;
				// p un-boosted：选最小算力的idle cpu
				if (!boosted && cpu_cap > target_cap)
					continue;
				// 选exit_latency 最小的cpu
				...
				// 非idle cpu中全范围（指所有pd覆盖的cpu中）找剩余算力最大的cpu
		...
		// 2. 对于每个pd中最大剩余算力的cpu，计算功耗，选功耗最小的CPU
...
unlock:
	...
	// 3.2延迟敏感，用最佳idle或者剩余算力最大的cpu（不考虑功耗了。。）
	if (latency_sensitive)
	 ...
	 */
	// prev cpu不满足选择时，直接选功耗最低的
	if (prev_delta == ULONG_MAX)
		return best_energy_cpu;

	// 功耗最低的cpu比prev cpu功耗低6%以上，选功耗最低的；
...
//		对于wakeup：是p阻塞之前的cpu
//		对于fork：是父进程所在的cpu
//		对于exec: 是fork选核后,exec选核前p所在的cpu
// sd_flag: SD_BALANCE_WAKE,SD_BALANCE_FORK,SD_BALANCE_EXEC
// wake_flag：WF_SYNC：A唤醒B后A马上阻塞，使用唤醒函数__wake_up_sync_key会带此flag
// 基本策略：见代码1,2,3
static int
...
no_eas:
		// 主要current与被唤醒任务p之间是否有比较固定的唤醒关系
		want_affine = !wake_wide(p) && cpumask_test_cpu(cpu, p->cpus_ptr);
	}

	rcu_read_lock();

	// 2. 处理want_affine, 和 找到支持sd_flag的最高层级的sd
	for_each_domain(cpu, tmp) {
		 ...
		 */
		// 2.1 current和p want_affine, this和prev cpu中选一个
		if (want_affine && (tmp->flags & SD_WAKE_AFFINE) &&
		    cpumask_test_cpu(prev_cpu, sched_domain_span(tmp))) {
			if (cpu != prev_cpu)
				// wake_affine在this_cpu和prev_cpu中选出一个作为候选
				new_cpu = wake_affine(tmp, p, cpu, prev_cpu, sync);

			sd = NULL; /* Prefer wake_affine over balance flags */
			break;
		}
		// 2.2 找到支持sd_flag的最高层级的sd
		if (tmp->flags & sd_flag)
			sd = tmp; 
		else if (!want_affine)
			break;
	...
	// 经过上面for循环：
	// 由于MC和DIE中不存在SD_BALANCE_WAKE => wakup case: sd==null
	// MC和DIE中都存在SD_BALANCE_FORK和SD_BALANCE_EXEC => fork/exec case: sd==DIE

	if (unlikely(sd)) {
		/* Slow path */
		// 3. fork和exec两类选核流程走slow path，选负载最低的cpu，
		// 如果负载差不多，优先考虑与this cpu share cache的
		new_cpu = find_idlest_cpu(sd, p, cpu, prev_cpu, sd_flag); // 这里sd是DIE sd
...
static void set_last_buddy(struct sched_entity *se)
{
	...
	if ((se->cfs_rq && se->cfs_rq->bursted) || (se->my_q && se->my_q->bursted))
		return;
...
static void check_preempt_wakeup(struct rq *rq, struct task_struct *p, int wake_flags)
{
	...
	if (cfs_rq->bursted && !cfs_rq_of(pse)->bursted)
		goto preempt;
...
int can_migrate_task(struct task_struct *p, struct lb_env *env)
{
	...
	if (!cpumask_test_cpu(env->dst_cpu, p->cpus_ptr)) {
		...
		// 任务由于affinity的原因不能在dest cpu上运行，
		// 因此这里设置上LBF_SOME_PINNED标志，表示至少有一个任务由于affinity无法迁移
		 ...
		 */
		// new idle不需要找备选，它任务是pull 1个task到this cpu
		// 如果已经LBF_DST_PINNED， 说明已经有了备选
		if (env->idle == CPU_NEWLY_IDLE || (env->flags & LBF_DST_PINNED))
			return 0;

		/* Prevent to re-select dst_cpu via env's CPUs: */
		// 在dst_cpu所在sg中找备选作为new_dst_cpu
	...
	// 4. p正在运行，不要migrate，迁移running task是后续active migration的逻辑
	...
	// 5. task非cache hot，或者balance failed次数太多，不管cache只能强制migrate
	if (tsk_cache_hot <= 0 ||
 ...
 */
// 主要作用：遍历rq上所有cfs tasks，找到最适合迁移到目标cpu rq的task，并挂入lb_env->tasks链表
// 返回迁出tasks个数
static int detach_tasks(struct lb_env *env)
{
	// src_rq（待迁出cpu）上所有cfs tasks
	...
	while (!list_empty(tasks)) {
		 ...
		 */
		// dst为idle，src只有一个在运行的task，不要把task从src迁到dst，否则会来回拉扯
		...
		// 任务不适合迁移，那么将其移到cfs_tasks链表头部
		if (!can_migrate_task(p, env))
			goto next;

		// 不平衡有几种类别，计算把task迁走后，计算env->imbalance的值，直到为0
		...
		case migrate_load:
			 ...
			 * migrate.
			 */
			// 不要迁移太多load，除非nr_balance_failed次数多
		 ...
		 */
		// newidle balance会带来延迟，所以1次只迁移1个
...
// p->se.avg.load_avg是在某cgroup层的相对负载
...
/*
 * CPU进入idle状态，并不意味着该CPU上的load avg的负载都是零，
 * 原来运行在该CPU的任务负载仍然存在（虽然这些任务阻塞了），只不过是需要不断进行衰减。
 * 因此，我们会在下面的场景中对CPU负载进行衰减：
 * a. Nohz idle balance（更新所有idle cpu的blocked load）
 * b. Newidle balance（更新本CPU的blocked load）
 * c. Load balance（更新本CPU的blocked load）
 */
...
// 更新算力
/*
在base domain（在手机平台上就是MC domain）上，我们会更新发起均衡所在CPU的算力。
注意：这里说的CPU算力指的是该CPU可以用于cfs任务的算力，即需要去掉由于thermal pressure而损失的算力，
去掉RT/DL/IRQ消耗的算力。在其他non-base domain（在手机平台上就是DIE domain）上，
我们需要对本地sched group（包括发起均衡的CPU所在的group）进行算力更新。
这个比较简单，就是把child domain（即MC domain）的所有sched group的算力加起来就OK了。
更新后的算力保存在sched group中的sgc成员中。
*/
 ...
 */
// cpu上cfs可用算力* imbalance_pct < 原始算力
// 说明被rt/dl/irq等占用了
static inline int
			  ...
			  struct sg_lb_stats *sgs)
{
	...
	// 该group中有misfit task需要迁移到算力更强的CPU上去
 ...
 * @env: The load balancing environment.
 * @sds: sched_domain statistics - sd domain负载数据
 * @sg: sched_group candidate to be checked for being the busiest - 当前sg
 * @sgs: sched_group statistics - 当前sg的负载数据
 *
					  ...
					  struct task_struct *p)
{
	...
	for_each_cpu(i, sched_group_span(group)) {
		struct rq *rq = cpu_rq(i);
		unsigned int local;
		// rq->cfs_rq->avg.load_avg - task_h_load(p)
...
// 主要功能：比较sgs是否比idlest_sgs更idle，是则返回true
 ...
 */
// 主要功能：从sched domain下的group找idlest group
// 主要策略：考虑两个：local group（this cpu所在的group），idlest的group
//			为什么要单独考虑local group，因为如果idlest group和local group load差不多
//			考虑到cpu cache，优先选local，只有idlest load比local少很多的时候，才选idlest。
// 返回值：null，选local group，否则为idlest group
static struct sched_group *
find_idlest_group(struct sched_domain *sd, struct task_struct *p, int this_cpu)
{
	...
	do {
		...
		if (!cpumask_intersects(sched_group_span(group),
					p->cpus_ptr))
			continue;
		// 1. 找到local_group，即this_cpu所在的group
	...
	// 3. 下面local group和idlest group开始pk，谁更合适？
 ...
 */
// 确定均衡类型 env->migration_type
// 量化得到不均衡数值 env->imbalance
static inline void calculate_imbalance(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	// 2. busiest group是因为cpu affinity而导致的不均衡，那么通过通过迁移任务来达到平衡，并且一次迁移一个任务
...
// Force：该sched domain处于不均衡状态，通过calculate_imbalance计算不均衡指数，
//		  并有可能通过任务迁移让系统进入均衡状态。
// Avg load：通过sched group的平均负载来判断是否需要均衡。
//			 尽量不均衡，除非非常的不均衡（通过sched domain的imbalance_pct参数来设定）
// Nr idle：dest cpu处于idle状态，并且local group的idle cpu个数大于busiest group的idle cpu个数，
//			只有在这种情况下才进行均衡。
 ...
 */
// 根据迁移类型，寻找busiest group中最繁忙的cpu
				     ...
				     struct sched_group *group)
{
	...
	for_each_cpu_and(i, sched_group_span(group), env->cpus) {
		 ...
		 * average load.
		 */
		// case: DIE sd: local=[0,1,2,3]，busiest=[4,5,6,7]
		// dst_cpu为1，则在busiest group遍历时，如果cpu上有一个task在运行
		// 不要向小核迁移
		if (env->sd->flags & SD_ASYM_CPUCAPACITY &&
		...
		case migrate_load:
			 ...
			 */
			// 1.1 迁移load：找出load/capacity 最大的rq
		...
		case migrate_misfit:
			 ...
			 */
			// 1.4 迁移misfit：找到misfit_task_load最大的rq
...
static int should_we_balance(struct lb_env *env)
{
	...
	// DIE sd下group多个cpu，只需要其中1个idle做balance
...
基本流程：
假设this_cpu=1; 
0.先在mc sd：{[1],[2],[3],[0]}中均衡，
1.找最buesiest的sg(find_busiest_group): 
	2.1 local_group=[1],假设计算出busiest_group=[3]
	2.2.计算mc sd中不均衡值(calculate_imbalance)放入env->imbalance
2.busiest group中找busiest的cpu(find_busiest_queue):
...
1.找最buesiest的sg(find_busiest_group): 
	...
	2.2.计算die sd中不均衡值(calculate_imbalance)放入env->imbalance
2.busiest group中找busiest的cpu(find_busiest_queue):
	假设是busiest->cpu=6
3. 从busiest->cpu==6 detach一些tasks，使env->imbalance降低为0，attach到this_cpu==1
*/
...
more_balance:
		...
		// 3.1 detach_tasks函数用来从busiest cpu的rq中摘取适合的任务
		...
		// 3.2 将detach_tasks函数摘下的任务挂入到dst rq上去
		if (cur_ld_moved) {
			attach_tasks(&env);
		...
		// 4.3 如果选中的busiest cpu上的任务全部都是通过affinity锁定在了该cpu上，
		// 那么清除该cpu（为了确保下轮均衡不考虑该cpu），再次发起均衡。
		// 这种情况下，需要重新搜索src cpu，因此跳转到redo
...
// 1. sd包含cpu越多interval越大
// 2. cpu busy, 间隔更大
static inline unsigned long
get_sd_balance_interval(struct sched_domain *sd, int cpu_busy)
{
	// 4+4大小核cpu，MC上interval=4~8ms
	// DIE上8~16ms
...
static void rebalance_domains(struct rq *rq, enum cpu_idle_type idle)
{
	...
	for_each_domain(cpu, sd) {
		 ...
		 */
		// 1. sd->max_newidle_lb_cost衰减，max_cost累加
		if (time_after(jiffies, sd->next_decay_max_lb_cost)) {
			// 1.1 sd->max_newidle_lb_cost表示在该sd上执行balance的最大开销
			// 每秒衰减1次，每次1%
...
static void kick_ilb(unsigned int flags)
{
	 ...
	 */
	// 更新nohz.next_balance到下一个jiffies。
	// 更新之后，其他的CPU的tick（同一个jiffies）将不会再触发nohz balance的检查
	 ...
	 */
	/*
	我们定义发起nohz idle balance的CPU叫做kicker；
	接收请求来执行均衡操作的CPU叫做kickee。Kicker和kickee之间的交互是这样的：
	a) Kicker通知kickee已经被选中执行nohz idle balance，
		具体是通过设定kickee cpu runqueue的nohz_flags成员来完成的。
	b) Send ipi把kickee唤醒
	c) Kickee被中断唤醒，执行scheduler_ipi来处理这个ipi中断。
		当发现其runqueue的nohz_flags成员被设定了，那么知道自己被选中，
		后续的流程其实和周期性均衡一样的，都是触发一次SCHED_SOFTIRQ类型的软中断。
	*/
 ...
 */
// 主要判断是否要唤醒nohz idle cpu去做load balance
static void nohz_balancer_kick(struct rq *rq)
{
	...
	// 本cpu是idle的，不需要触发nohz idle balance
	...
	// 执行 _nohz_idle_balance 的时候，调度器会把idle cpus中rq->next_balance最小的
	// 赋值给nohz.next_balance，这个值作为触发nohz idle balance的时间点
	...
	// 本身任务较多，那就触发balance，或许可以让idle的cpu帮忙
	if (rq->nr_running >= 2) {
	...
	if (sd) {
		 ...
		 * on.
		 */
		// 本身有至少1个cfs task，且cfs可用算力有降低(被其他rt/dl/irq占用),要触发
	...
	if (sd) {
		 ...
		 */
		/*
		对于异构系统，我们忽略了LLC check，这是因为功耗的考量。
		小核cluster有busy的CPU并不说明需要进行均衡，只要小核CPU有足够的算力能够容纳当前运行的任务，
		那么没有必要发起nohz idle balance把大核给搞起来，增加额外的功耗。
		*/
		goto unlock;
	}

	// 同构系统中，有SD_SHARE_PKG_RESOURCES flag的最高sd
	// llc: last level cache, 指共享3级缓存的sd
	sds = rcu_dereference(per_cpu(sd_llc_shared, cpu));
	if (sds) {
		/*
 ...
 */
// 主要功能：
// 1. 更新所有nohz idle cpus上的blocked负载(NOHZ_BALANCE_KICK|NOHZ_STATS_KICK)
// 2. 负载均衡，给所有nohz idle cpus拉任务(need flag：NOHZ_BALANCE_KICK)
//		为什么在负载均衡路径里更新blocked负载？因为cpu已经idle没有task活动，没有更新负载的入口了
			       ...
			       enum cpu_idle_type idle)
{
	...
	// 1. 除this cpu，对于其他nohz idle cpus，更新blocked负载，做负载均衡
	for_each_cpu(balance_cpu, nohz.idle_cpus_mask) {
		...
		if (time_after_eq(jiffies, rq->next_balance)) {
			...
			// 1.2 负载均衡，拉任务到nohz idle cpu，让它干活
			if (flags & NOHZ_BALANCE_KICK)
				rebalance_domains(rq, CPU_IDLE);
		}

		// 取最小的rq->next_balance，后赋给nohz.next_balance
	...
	// 2. 更新this cpu的blocked负载，newly idle不需要，因为在执行
	if (idle != CPU_NEWLY_IDLE) {
...
static void nohz_newidle_balance(struct rq *this_rq)
{
	 ...
	 */
	// 更新nohz idle cpus的blocked负载，如果不成功，则唤醒一个nohz idle cpu去更新
 ...
 */
//New idle balance可能影响延迟，每次cpu进入idle就会触发，需要更加谨慎对待
static int newidle_balance(struct rq *this_rq, struct rq_flags *rf)
{
	...
	// avg_idle时间大于迁移消耗时间(提升延迟，造成cache cold) || 所在整个系统都没有overload
	// 1. 不做load balance，但是要干一个活：更新所有idle cpus上的blocked负载
	if (this_rq->avg_idle < sysctl_sched_migration_cost ||
	...
	update_blocked_averages(this_cpu);
	rcu_read_lock();
	// 从this cpu的MC->DIE做负载均衡
	for_each_domain(this_cpu, sd) {
		int continue_balancing = 1;
		u64 t0, domain_cost;

		// newidle balance对延迟有影响，严格检查
